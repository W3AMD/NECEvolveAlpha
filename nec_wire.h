...
class nec_wire {
public:
	nec_wire(const nec_3vector& a, const nec_3vector& b, nec_float in_radius,
		int id) : x0(a), x1(b), radius(in_radius), _tag_id(id) {
	}
	...
	std::vector<nec_wire>intersect(nec_wire& b) {
		...
		nec_float epsa = b.radius / length();
		// Set by the radius of the other wires
		nec_float epsb = radius / b.length();
		// Set by the radius of the other wires

		std::vector<nec_wire>ret;

		if (d2 > (radius + b.radius))
			return ret;

		if ((sa >= -epsa) && (sa <= 1.0 + epsa) && (sb >= -epsb) && (sb <=
			1.0 + epsb)) {
			nec_3vector a_pt = parametrize(sa);
			nec_3vector b_pt = b.parametrize(sb);

			if (sa > epsa)
				ret.push_back(nec_wire(x0, a_pt, radius, _tag_id));
			if (sa < 1.0 - epsa)
				ret.push_back(nec_wire(a_pt, x1, radius, _tag_id));

			if (sb > epsb)
				ret.push_back(nec_wire(b.x0, b_pt, b.radius, b._tag_id));
			if (sb < 1.0 - epsb)
				ret.push_back(nec_wire(b_pt, b.x1, b.radius, b._tag_id));
		}
	...
	bool intersect(nec_3vector& b0) {
		...
		/*
		 ...
		 print solution
		 */
		nec_float sa = (a1x * b0x + a1y * b0y + a1z * b0z - a0x * a1x -
			a0x * b0x - a0y * a1y - a0y * b0y - a0z * a1z - a0z * b0z +
			a0x * a0x + a0y * a0y + a0z * a0z) /
			(-2.0 * a0x * a1x - 2 * a0y * a1y - 2.0 * a0z * a1z + a0x * a0x +
			a0y * a0y + a0z * a0z + a1x * a1x + a1y * a1y + a1z * a1z);
		if (sa < 0)
			sa = 0.0;
		if (sa > 1.0)
			sa = 1.0;
	 ...
	 */
	static void int_solve(nec_3vector& a0, nec_3vector& a1, nec_3vector& b0,
		nec_3vector& b1, nec_float& distance, nec_float& sa, nec_float& sb) {
		nec_float a0x = a0.x();
		nec_float a0y = a0.y();
		nec_float a0z = a0.z();
		nec_float b0x = b0.x();
		...
		nec_float tmp3 =
			(-4.0 * (a0x * a0x + a0y * a0y + a1x * b0x - a0x * (a1x + b0x) +
			a1y * b0y - a0y * (a1y + b0y) + a01z * (a0z - b0z)) * tmp2 +
			4.0 * moda * ((a0x - b0x) * b01x + (a0y - b0y) * b01y +
			(a0z - b0z) * b01z));

		sa = (a0x * a01x + a0y * a01y + a0z * a01z - a01x * b0x - a01y * b0y -
			a01z * b0z - tmp3 * tmp2 / den) / moda;
