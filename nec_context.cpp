...
#include <cstdlib>
extern std::vector<nec_float>XCenterSeg, YCenterSeg, ZCenterSeg;
...
nec_float nec_context::benchmark() {
	...
	for (int i = 0; i < 20; i++) { {
			...
			geo->wire(0, 8, 0.0, 0.0, -0.25, 0.0, 0.0, 0.25, 0.00001, 1.0, 1.0);
 ...
 */
void nec_context::wire(int tag_id, int segment_count, nec_float xw1, nec_float yw1, nec_float zw1, nec_float xw2,
  nec_float yw2, nec_float zw2, nec_float rad, nec_float rdel, nec_float rrad) {
	m_geometry->wire(tag_id, segment_count, xw1, yw1, zw1, xw2, yw2, zw2, rad, rdel, rrad);
}

void nec_context::sp_card(int ns, nec_float x1, nec_float y1, nec_float z1, nec_float x2, nec_float y2, nec_float z2) {
	m_geometry->sp_card(ns, x1, y1, z1, x2, y2, z2);
}

void nec_context::sc_card(int i2, nec_float x3, nec_float y3, nec_float z3, nec_float x4, nec_float y4, nec_float z4) {
...
void nec_context::move(nec_float rox, nec_float roy, nec_float roz, nec_float xs, nec_float ys, nec_float zs, int its,
  int nrpt, int itgi) {
 ...
 */
void nec_context::arc(int tag_id, int segment_count, nec_float rada, nec_float ang1, nec_float ang2, nec_float rad) {
...
void nec_context::ld_card(int itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2, nec_float tmp3) {
	...
	if (itmp4 < itmp3) {
		nec_stop("DATA FAULT ON LOADING CARD NO: %d: ITAG " "STEP1: %d IS GREATER THAN ITAG STEP2: %d", nload,
		  itmp3, itmp4);
	}
 ...
 */
void nec_context::gn_card(int ground_type, int rad_wire_count, nec_float tmp1, nec_float tmp2, nec_float tmp3,
 ...
 */
void nec_context::ex_card(enum excitation_type itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2,
  nec_float tmp3, nec_float tmp4, nec_float tmp5, nec_float tmp6) {
	DEBUG_TRACE("ex_card(" << itmp1 << "," << itmp2 << "," << itmp3 << "," << itmp4 << "," << tmp1 << "," << tmp2 <<
	  "," << tmp3 << "," << tmp4 << "," << tmp5 << "," << tmp6 << ")");
 ...
 */
void nec_context::tl_card(int itmp1, int itmp2, int itmp3, int itmp4, nec_float characteristic_impedance,
  nec_float tmp2, nec_float tmp3, nec_float tmp4, nec_float tmp5, nec_float tmp6) {
	if (iflow != 6) {
		network_count = 0;
		ntsol = 0;
		iflow = 6;
 ...
 */
void nec_context::nt_card(int itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2, nec_float tmp3,
  nec_float tmp4, nec_float tmp5, nec_float tmp6) {
	if (iflow != 6) {
		network_count = 0;
		ntsol = 0;
...
void nec_context::xq_card(int itmp1) {
	...
	if (((iflow == 10) && (itmp1 == 0)) || ((nfrq == 1) && (itmp1 == 0) && (iflow > 7)))
		return; /*continue card input loop*/
 ...
 */
void nec_context::rp_card(int calc_mode, int n_theta, int n_phi, int output_format, int normalization, int D, int A,
  nec_float theta0, nec_float phi0, nec_float delta_theta, nec_float delta_phi, nec_float radial_distance,
  nec_float gain_norm) {
...
void nec_context::ne_card(int itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2, nec_float tmp3,
  nec_float tmp4, nec_float tmp5, nec_float tmp6) {
	ne_nh_card(0, itmp1, itmp2, itmp3, itmp4, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6);
}

void nec_context::nh_card(int itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2, nec_float tmp3,
  nec_float tmp4, nec_float tmp5, nec_float tmp6) {
 ...
 */
void nec_context::ne_nh_card(int in_nfeh, int itmp1, int itmp2, int itmp3, int itmp4, nec_float tmp1, nec_float tmp2,
  nec_float tmp3, nec_float tmp4, nec_float tmp5, nec_float tmp6) {
	nfeh = in_nfeh;

	if ((iflow == 8) && (nfrq != 1)) {
		m_output.endl(2);
		m_output.line("WHEN MULTIPLE FREQUENCIES ARE REQUESTED, " "ONLY ONE NEAR FIELD CARD CAN BE USED -");
...
void nec_context::simulate(bool far_field_flag) {
	...
	if ((far_field_flag == true) && (processing_state == 5))
		igox = 6;
	else
	...
	try {
		...
		do {
			...
			case 2: /*structure segment loading*/ {
					...
					unsigned int segloadtime = end - start;
					processing_state = 3;
					ntsol = 0;
				}
			...
			default:
				...
				if (FREQ_LOOP_CARD_CONTINUE == ret) {
					break;
					// original don't know why this wasn't break to begin with
...
void nec_context::print_freq_int_krnl(nec_float f, nec_float lambda, nec_float int_dist, bool using_extended_kernel) {
	m_output.endl(5);
	m_output.set_indent(33);
	m_output.line("- - - - - - FREQUENCY - - - - - -\n");
	m_output.set_indent(36);
	m_output.string("FREQUENCY=");
	m_output.real_out(11, 4, f);
	m_output.line(" MHZ");
	m_output.string("WAVELENGTH=");
	m_output.real_out(11, 4, lambda);
	m_output.line(" METERS");
	m_output.endl(4);

	m_output.set_indent(20);
	char outLine[255];
	sprintf(outLine, "APPROXIMATE INTEGRATION EMPLOYED FOR SEGMENTS MORE THAN   %5.3f WAVELENGTHS APART\n", int_dist);
	// m_output.line("APPROXIMATE INTEGRATION EMPLOYED FOR SEGMENTS MORE THAN   ");
	// m_output.real_out(5, 3, int_dist, false);
	m_output.line(outLine);
...
void nec_context::antenna_env(void) {
	unsigned int start = GetTickCount();
	ground.calculate_antenna_environment(ground_wave, freq_mhz); // 985 ms
	unsigned int end = GetTickCount();
	unsigned int calantenvtime = end - start;
	start = GetTickCount();
	ground.output_antenna_environment(m_output); // 56 ms
	end = GetTickCount();
	unsigned int groundoutputantenvtime = end - start;
}
...
void nec_context::calculate_network_data(void) {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (itmp3 == 0)
		  // can only be zero if all the networks are the same type
			  return;
		net_type = itmp3;
	}
	...
	if ((network_count != 0) && (inc <= 1)) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (itmp1 == 2) {
				m_output.endl();
				m_output.nec_printf
				  ("  -- FROM -  --- TO --      " "TRANSMISSION LINE       " " --------- SHUNT ADMITTANCES (MHOS) "
			...
			for (int j = 0; j < network_count; j++) {
				...
				if ((itmp2 / itmp1) != 1) {
					...
					idx5 = itmp5 - 1;
					m_output.endl();
					m_output.nec_printf(" %4d %5d %4d %5d  " "%11.4E %11.4E  %11.4E %11.4E  " "%11.4E %11.4E %s",
					  m_geometry->segment_tags[idx4], itmp4, m_geometry->segment_tags[idx5], itmp5, x11r[j], x11i[j],
					  x12r[j], x12i[j], x22r[j], x22i[j], pnet[itmp2 - 1]);
				}/*if (( itmp2/ itmp1) == 1)*/
...
void nec_context::print_norm_rx_pattern() {
...

	{
		...
		nec_norm_rx_pattern* rx_pattern = new nec_norm_rx_pattern(nthi, nphi, fnorm, thetis, theta_step, phiss,
		  phi_step, eta, axial_ratio, segment_number, pol_type);
	...
	m_output.nec_printf
	  ...
	  "TYPE: %s\n" "                      AXIAL RATIO: %6.3f\n" "                      SEGMENT NO: %d\n\n"
	  "                      " "THETA     PHI       ---- PATTERN ----\n" "                      "
	  "(DEG)    (DEG)       DB     MAGNITUDE", norm_factor, xpr3, hpol[m_excitation_type - 1], xpr6, isave);
	...
	for (int j = 0; j < nphi; j++) {
		...
		for (int i = 0; i < nthi; i++) {
			...
			if (itmp3 < itmp1) {
				...
				m_output.endl();
				m_output.nec_printf("                    %7.2f  %7.2f   %7.2f  %11.4E", xpr1, phi, _tmp3, _tmp2);
...
void nec_context::print_power_budget(void) {
	if ((m_excitation_type == EXCITATION_VOLTAGE) || (m_excitation_type == EXCITATION_VOLTAGE_DISC)) {
		...
		m_output.nec_printf
		  ...
		  "                                           EFFICIENCY    =%7.2f PERCENT", input_power, radiated_power,
		  structure_power_loss, network_power_loss, efficiency);
		m_output.endl(4);
...
void nec_context::print_input_impedance() {
	...
	if (iped != 0) {
		...
		m_output.nec_printf
		  ...
		  "     OHMS         OHMS         OHMS     DEGREES", iss, impedance_norm_factor);
		...
		for (int i = 0; i < nfrq; i++) {
			...
			m_output.endl();
			m_output.nec_printf(" %9.3f   %11.4E  %11.4E  %11.4E  %7.2f  " " %11.4E  %11.4E  %11.4E  %7.2f", tmp1,
			  fnorm(i, 0), fnorm(i, 1), fnorm(i, 2), fnorm(i, 3), tmp2, tmp3, tmp4, tmp5);
...
void nec_context::structure_segment_loading() {
	...
	unsigned int start = GetTickCount();
	antenna_env(); // 969 ms typical
	unsigned int end = GetTickCount();
	unsigned int antenna_envtime = end - start;

	/*fill and factor primary interaction matrix*/
	secnds(&tim1);
	start = GetTickCount();
	cmset(neq, cm, rkh); // 450 ms typical
	end = GetTickCount();
	unsigned int cmsettime = end - start;
	secnds(&tim2);
	tim = tim2 - tim1;
	start = GetTickCount();
	factrs(m_output, npeq, neq, cm, ip); // 15 ms typical
	end = GetTickCount();
	unsigned int factrstime = end - start;
	secnds(&tim1);
	tim2 = tim1 - tim2;

	m_output.end_section();
	m_output.line("                                - - - MATRIX TIMING - - -\n");
	m_output.string("                        FILL=    ", false);
	m_output.integer(int(tim));
	m_output.string(" msec  FACTOR: ");
	m_output.integer(int(tim2));
	m_output.string(" msec");
	m_output.end_section();
}
...
void nec_context::setup_excitation() {
	...
	if ((m_excitation_type != 0) && (m_excitation_type != 5)) {
		...
		if (m_excitation_type == 4) {
			...
			m_output.nec_printf("               %10.5f %10.5f %10.5f  %7.2f     %7.2f    %8.3f", xpr1, xpr2, xpr3, xpr4,
			  xpr5, xpr6);
		}
		else {
			...
			if (iptflg <= 0) {
				m_output.endl();
				m_output.nec_printf
				  ("PLANE WAVE - THETA: %7.2f deg, PHI: %7.2f deg," " ETA=%7.2f DEG, TYPE - %s  AXIAL RATIO: %6.3f",
				  xpr1, xpr2, xpr3, nec_structure_currents::hpol(m_excitation_type).c_str(), xpr6);
			}
		}/*if ( m_excitation_type == 4)*/
	}

	nec_float incident_amplitude = 1.0;
	// the incident electric field amplitude...
	if (xpr7 != 0.0)
...
enum excitation_return nec_context::excitation_loop(int in_freq_loop_state, int mhz) {
	...
	do {
		if (in_freq_loop_state < 4) {
			...
			if ((iptflg < -2) || (iptflg > 3)) {
				m_output.line
				  ("WARNING : The print control flag for currents was uncorrect ; it has been set to -1 (no printing for currents)."
				  );
				iptflg = -1;
			}
			...
			if ((iptflq < -2) || (iptflq > 0)) {
				m_output.line
				  ("WARNING : The print control flag for charge densities was uncorrect ; it has been set to -1 (no printing for charge densities)."
				  );
				iptflq = -1;
			}
			...
			if (m_excitation_type != 1 && m_excitation_type != 2 && m_excitation_type != 3) {
				if (iptflg == 2 || iptflg == 3) {
					m_output.line
					  ("WARNING : The output format chosen for currents was incompatible with the excitation type. No currents has been printed."
					  );
					iptflg = -1;
				}
			}
			...
			if (iptflg != -1 || iptflq != -1) {
				if ((iptflg < 1 || iptflg == 3 || ((iptflg == 1 || iptflg == 2) && inc <= 1)) ||
				  (iptflg == -1 && iptflq != -1)) {
					structure_currents = new nec_structure_currents(this, m_excitation_type, nload, xpr3, xpr6);
		...
		if (ifar != -1) {
			nec_radiation_pattern* rad_pat = new nec_radiation_pattern(nth, nph, thets, phis, dth, dph, rfld, ground,
			  ifar, _wavelength, input_power, network_power_loss, m_rp_output_format, m_rp_normalization, ipd, iavp,
			  gnor, plot_card);
...
void nec_context::load() {
	...
	while (true) {
		...
		if (istep > nload) {
			if (iwarn == true)
				m_output.line("WARNING: SOME OF THE ABOVE SEGMENTS HAVE BEEN LOADED TWICE - IMPEDANCES ADDED");
		...
		if (ldtyp[istepx] > 5) {
			nec_stop("ERROR: IMPROPER LOAD TYPE CHOSEN," " REQUESTED TYPE IS %d", ldtyp[istepx]);
		}
		...
		if (ichk == 0) {
			nec_exception* nex = new nec_exception("ERROR: LOADING DATA CARD ERROR, NO SEGMENT HAS AN ITAG = ");
			nex->append(ldtags);
			throw nex;
		}
		...
		switch (jump) {
		case 1:
			impedance_print(ldtags, ldtagf[istepx], ldtagt[istepx], zlr[istepx], zli[istepx], zlc[istepx], 0., 0., 0.,
			  " SERIES ");
			break;
		...
		case 5:
			impedance_print(ldtags, ldtagf[istepx], ldtagt[istepx], 0., 0., 0., zlr[istepx], zli[istepx], 0.,
			  "FIXED IMPEDANCE ");
			break;

		case 6:
			impedance_print(ldtags, ldtagf[istepx], ldtagt[istepx], 0., 0., 0., 0., 0., zlr[istepx], "  WIRE  ");
		}/*switch( jump )*/
...
void nec_context::cmset(int64_t nrow, complex_array& in_cm, nec_float rkhx) {
	...
	for (int i = 0; i < it; i++) {
		...
		for (int j = 0; j < npeq; j++) {
			...
			in_cm[row_offset + j] = scm.segment(0, nop).sum();
			// scm.sum(0,nop);
...
void nec_context::compute_matrix_ss(int j1, int j2, int im1, int im2, complex_array& in_cm, int64_t nrow, int itrp) {
...
void nec_context::cmsw(int j1, int j2, int i1, int i2, complex_array& in_cm, complex_array& cw, int64_t ncw,
  int64_t nrow, int itrp) {
	nec_float fsign = 1.0;
			...
			for (int ipgnd = 1; ipgnd <= ground.ksymp; ipgnd++) {
				if (((ipch == j) || (icgo != 0)) && (ipgnd != 2)) {
					if (icgo <= 0) {
						...
						if (itrp == 0)
							cw[k + il * nrow] +=
							  exc * (m_geometry->ax[jsnox] + m_geometry->bx[jsnox] * pxl + m_geometry->cx[jsnox] * pyl);
						else
							cw[il + k * nrow] +=
							  exc * (m_geometry->ax[jsnox] + m_geometry->bx[jsnox] * pxl + m_geometry->cx[jsnox] * pyl);
					}/*if ( icgo <= 0 )*/
...
void nec_context::cmws(int j, int i1, int i2, complex_array& in_cm, int64_t nr, complex_array& cw, int64_t nw, int itrp)
{
	UNUSED(nw);
	int ipr, js = 0;
	...
	for (int i = i1; i <= i2; i++) {
		...
		case 2: /*transposed fill - c(ws) and d(ws)prime (=cw)*/
			for (int ij = 0; ij < m_geometry->jsno; ij++) {
				int jx = m_geometry->jco[ij] - 1;
				if (jx < nr) {
					in_cm[jx + ipr * nr] += etk * m_geometry->ax[ij] + ets * m_geometry->bx[ij] +
					  etc * m_geometry->cx[ij];
				}
				else {
					jx -= nr;
...
void nec_context::cmww(int j, int i1, int i2, complex_array& in_cm, int64_t nr, complex_array& cw, int64_t nw, int itrp)
{
	UNUSED(nw);
	int i, jx;
	...
	if (m_use_exk == true) {
...
#if 0
		if (ipr > PCHCON)
			ind1 = 0; // trap bug also in original fortran
		else if (ipr == 0)
			ind1 = 1;
		...
		if (ipr > PCHCON)
			ind2 = 2; // trap bug also in original fortran
		else if (ipr == 0)
			ind2 = 1;
		else if (ipr < 0) {
			int iprx = -ipr - 1;
...
void nec_context::couple(complex_array& in_currents, nec_float in_wavelength) {
	...
	m_output.nec_printf
	  ...
	  " TAG  SEG   No:   TAG  SEG   No:      (DB)       " " REAL     IMAGINARY         REAL       IMAGINARY");
	...
	for (int i = 0; i < npm1; i++) {
		...
		for (int j = l1; j < ncoup; j++) {
			...
			if ((c >= 0.0) && (c <= 1.0)) {
				...
				m_output.endl();
				m_output.nec_printf(" %4d %4d %5d  %4d %4d %5d  %9.3f" "  %12.5E %12.5E  %12.5E %12.5E", itt1, its1,
				  isg1, itt2, its2, isg2, dbc, real(zl), imag(zl), real(zin), imag(zin));
			...
			m_output.endl();
			m_output.nec_printf(" %4d %4d %5d   %4d %4d %5d  **ERROR** " "COUPLING IS NOT BETWEEN 0 AND 1. (= %12.5E)",
 ...
 */
void nec_context::eksc(nec_float s, nec_float z, nec_float rh, nec_float xk, int ij, nec_complex *in_ezs,
  nec_complex *ers, nec_complex *in_ezc, nec_complex *erc, nec_complex *in_ezk, nec_complex *erk) {
	...
	*in_ezk = -__const1 * (gzp2 - gzp1 + xk * xk * nec_complex(cint, -sint));
	...
	if (rh >= 1.0e-10) {
		...
		*ers = -__const1 * ((gzp2 + gzp1 + gz2 + gz1) * ss - (z2a * gz2 - z1a * gz1) * cs * xk) / rh;
		*erc = -__const1 * ((gzp2 - gzp1 + gz2 - gz1) * cs + (z2a * gz2 + z1a * gz1) * ss * xk) / rh;
		return;
	}
 ...
 */
void nec_context::ekscx(nec_float bx, nec_float s, nec_float z, nec_float rhx, nec_float xk, int ij, int inx1, int inx2,
  nec_complex *in_ezs, nec_complex *ers, nec_complex *in_ezc, nec_complex *erc, nec_complex *in_ezk, nec_complex *erk) {
	static nec_complex __const1(0.0, 4.771341189);
	int ira;
	nec_float b, rh, sh, rhk, shk, ss, cs, z1a;
	nec_float z2a, a2, bk, bk2, cint, sint;
	nec_complex gz1, gz2, gzp1, gzp2, gr1, gr2;
	nec_complex nec_grp1, nec_grp2, grk1, grk2, gzz1, gzz2;
	...
	if (inx1 != 2)
		gxx(z1a, rh, b, a2, xk, ira, &gz1, &gzp1, &gr1, &nec_grp1, &grk1, &gzz1);
	else {
	...
	if (inx2 != 2)
		gxx(z2a, rh, b, a2, xk, ira, &gz2, &gzp2, &gr2, &nec_grp2, &grk2, &gzz2);
	else {
	...
	*in_ezs = __const1 * ((gz2 - gz1) * cs * xk - (gzp2 + gzp1) * ss);
	*in_ezc = -__const1 * ((gz2 + gz1) * ss * xk + (gzp2 - gzp1) * cs);
	*ers = -__const1 * ((z2a * nec_grp2 + z1a * nec_grp1 + gr2 + gr1) * ss - (z2a * gr2 - z1a * gr1) * cs * xk);
	*erc = -__const1 * ((z2a * nec_grp2 - z1a * nec_grp1 + gr2 - gr1) * cs + (z2a * gr2 + z1a * gr1) * ss * xk);
	*erk = __const1 * (grk2 - grk1);
	intx(-shk, shk, rhk, ij, &cint, &sint);
	bk = b * xk;
	bk2 = bk * bk * .25;
	*in_ezk = -__const1 * (gzp2 - gzp1 + xk * xk * (1. - bk2) * nec_complex(cint, -sint) - bk2 * (gzz2 - gzz1));
}
 ...
 */
void nec_context::etmns(nec_float p1, nec_float p2, nec_float p3, nec_float p4, nec_float p5, nec_float p6,
  nec_float incident_amplitude, enum excitation_type excite_type, complex_array& e) {
	...
	if ((excite_type == EXCITATION_LINEAR) || (excite_type == EXCITATION_CIRC_RIGHT) ||
	  (excite_type == EXCITATION_CIRC_LEFT)) {
		...
		if (excite_type == EXCITATION_LINEAR) {
			if (n != 0) {
				for (int i = 0; i < n; i++) {
					...
					e[i] = -(pxl * m_geometry->cab[i] + pyl * m_geometry->sab[i] + pzl * m_geometry->salp[i])
					  * e_amplitude * incident_amplitude;
				}
				...
				if (ground.present()) {
					...
					for (int i = 0; i < n; i++) {
						nec_float arg =
						  -two_pi() * (wx * m_geometry->x[i] + wy * m_geometry->y[i] - wz * m_geometry->z[i]);
						nec_complex e_amplitude(cos(arg), sin(arg));
...

			{
				...
				for (int is = 0; is < m; is++) {
					...
					nec_complex e_amplitude =
					  nec_complex(cos(arg), sin(arg)) * m_geometry->psalp[i] * em::inverse_impedance();
					e[i2] = (qx * m_geometry->t1x[i] + qy * m_geometry->t1y[i] + qz * m_geometry->t1z[i])
					  * e_amplitude * incident_amplitude;
					e[i1] = (qx * m_geometry->t2x[i] + qy * m_geometry->t2y[i] + qz * m_geometry->t2z[i])
					  * e_amplitude * incident_amplitude;
				}
			}
			...
			if (ground.present()) {
				...
				for (int is = 0; is < m; is++) {
					...
					e[i2] = e[i2] + (cx * m_geometry->t1x[i] + cy * m_geometry->t1y[i] + cz * m_geometry->t1z[i])
					  * e_amplitude * incident_amplitude;
					e[i1] = e[i1] + (cx * m_geometry->t2x[i] + cy * m_geometry->t2y[i] + cz * m_geometry->t2z[i])
					  * e_amplitude * incident_amplitude;
				}
			}
			return;
		...
		if (n != 0) {
			...
			if (ground.present()) {
				...
				for (int i = 0; i < n; i++) {
					nec_float arg = -two_pi() * (wx * m_geometry->x[i] + wy * m_geometry->y[i] - wz * m_geometry->z[i]);
					nec_complex e_amplitude(cos(arg), sin(arg));
					e[i] -= (ccx * m_geometry->cab[i] + ccy * m_geometry->sab[i] + ccz * m_geometry->salp[i])
					  * e_amplitude * incident_amplitude;
				}
			}
		}/*if ( n != 0)*/
...

		{
			...
			for (int is = 0; is < m; is++) {
				...
				e[i2] = (cx * m_geometry->t1x[i] + cy * m_geometry->t1y[i] + cz * m_geometry->t1z[i])
				  * e_amplitude * incident_amplitude;
				e[i1] = (cx * m_geometry->t2x[i] + cy * m_geometry->t2y[i] + cz * m_geometry->t2z[i])
				  * e_amplitude * incident_amplitude;
			}
		}
	...
	for (int i = 0; i < m_geometry->n_plus_m(); i++) {
		...
		else // patches
		{
			...
			e[i2] = cx * m_geometry->t1x[patch_index] + cy * m_geometry->t1y[patch_index] +
			  cz * m_geometry->t1z[patch_index];
			e[i1] = cx * m_geometry->t2x[patch_index] + cy * m_geometry->t2y[patch_index] +
			  cz * m_geometry->t2z[patch_index];
		}/*if ( i < n)*/
 ...
 */
void nec_context::gx(nec_float zz, nec_float rh, nec_float xk, nec_complex *gz, nec_complex *gzp) {
 ...
 */
void nec_context::gxx(nec_float zz, nec_float rh, nec_float a, nec_float a2, nec_float xk, int ira, nec_complex *g1,
  nec_complex *g1p, nec_complex *g2, nec_complex *g2p, nec_complex *g3, nec_complex *gzp) {
 ...
 */
void nec_context::hfk(nec_float el1, nec_float el2, nec_float rhk, nec_float zpkx, nec_float *sgr, nec_float *sgi) {
...
void nec_context::hsflx(nec_float s, nec_float rh, nec_float zpx, nec_complex *hpk, nec_complex *hps, nec_complex *hpc)
{
 ...
 */
void nec_context::nefld(nec_float xob, nec_float yob, nec_float zob, nec_complex *ex, nec_complex *ey, nec_complex *ez)
{
	int i, ix, ipr, jc;
	nec_float zp, ax;
...
/*present. */
void nec_context::netwk(complex_array& in_cm, int_array& in_ip, complex_array& einc)
  /*
   ...
   */ {
	...
	if (ntsol == 0) {
		...
		if (masym != 0) {
			...
			for (i = 0; i < voltage_source_count; i++) {
				...
				if (irow1 == 0) {
					/*TODO :
					 very large numbers in voltage excitation can cause access violation here
					 Example: EX,0,0,1,6881400,1.0000,0.0000,0*/
					ipnt[irow1] = nseg1;
					irow1++;
					continue;
				}
			...
			if (irow1 >= 2) {
				for (i = 0; i < irow1; i++) {
					...
					rhs[isc1] = cplx_10();
					solves(in_cm, in_ip, rhs, neq, 1, m_geometry->np, m_geometry->n_segments, m_geometry->mp,
					  m_geometry->m, nop, symmetry_array);
					m_geometry->get_current_coefficients(_wavelength, rhs, air, aii, bir, bii, cir, cii, vqds,
					  nqds, iqds);
				...
				m_output.nec_printf
				  ...
				  "   MATRIX IS %10.3E FOR SEGMENTS %d AND %d\n" "   RMS RELATIVE ASYMMETRY IS %10.3E", asmx, nteq,
				  ntsc, asa);
			}/*if ( irow1 >= 2)*/
		}/*if ( masym != 0)*/
		...
		if (network_count != 0) {
			...
			for (i = 0; i < nteq; i++) {
				...
				irow1 = nteqa[i] - 1;
				rhs[irow1] = cplx_10();
				solves(in_cm, in_ip, rhs, neq, 1, m_geometry->np, m_geometry->n_segments, m_geometry->mp, m_geometry->m,
				  nop, symmetry_array);
	...
	if (0 == network_count) {
		...
		solves(in_cm, in_ip, einc, neq, 1, m_geometry->np, m_geometry->n_segments, m_geometry->mp, m_geometry->m, nop,
		  symmetry_array);
	...
	else // if ( network_count != 0)
	{
		...
		solves(in_cm, in_ip, rhs, neq, 1, m_geometry->np, m_geometry->n_segments, m_geometry->mp, m_geometry->m, nop,
		  symmetry_array);
		m_geometry->get_current_coefficients(_wavelength, rhs, air, aii, bir, bii, cir, cii, vqds, nqds, iqds);
	...
	for (i = 0; i < voltage_source_count; i++) {
		...
		/*TODO : The following function is where the antenna input is set
		 into the vectors for impedance etc. This is the data that needs to be
		 copied into structures*/
		antenna_input->set_input(segment_tag, segment_index + 1, voltage, current, impedance, admittance, power);
 ...
 */
void nec_context::nhfld(nec_float xob, nec_float yob, nec_float zob, nec_complex *hx, nec_complex *hy, nec_complex *hz)
{
	*hx = cplx_00();
	*hy = cplx_00();
	*hz = cplx_00();
 ...
 */
void nec_context::pcint(nec_float xi, nec_float yi, nec_float zi, nec_float cabi, nec_float sabi, nec_float salpi,
  complex_array& e) {
...
void nec_context::impedance_print(int in1, int in2, int in3, nec_float fl1, nec_float fl2, nec_float fl3, nec_float fl4,
  nec_float fl5, nec_float fl6, const char *ia) {
...
void nec_context::qdsrc(int is, nec_complex v, complex_array& e) {
	...
	curd = s_CCJ * v / ((log(2. * m_s / m_geometry->segment_radius[is]) - 1.) *
	  (m_geometry->bx[m_geometry->jsno - 1] * cos(two_pi() * m_s) + m_geometry->cx[m_geometry->jsno - 1] * sin
	  (two_pi() * m_s)) * _wavelength);
	vqds[nqds] = v;
	iqds[nqds] = is + 1;
	nqds++;
	...
	for (jx = 0; jx < m_geometry->jsno; jx++) {
		...
		if (m != 0) {
			...
			for (i = 0; i < m; i++) {
				...
				e[i1] += (etk * m_geometry->ax[jx] + ets * m_geometry->bx[jx] + etc * m_geometry->cx[jx])
				  * curd * m_geometry->psalp[i];
				i1++;
				tx = m_geometry->t1x[i];
				ty = m_geometry->t1y[i];
				tz = m_geometry->t1z[i];
...
#define g(d)  ( exp(cn*(d)+th(8./x))/sqrt(two_pi()*(d)) )
	...
	if (x <= 110.) {
		if (x <= 8.) {
			...
			ber = ((((((-9.01e-6 * s + 1.22552e-3) * s - .08349609) * s + 2.6419140) * s - 32.363456) * s + 113.77778)
			  * s - 64.) * s + 1.;

			bei = ((((((1.1346e-4 * s - .01103667) * s + .52185615) * s - 10.567658) * s + 72.817777) * s - 113.77778)
			  * s + 16.) * y;

			br1 = nec_complex(ber, bei);

			ber = (((((((-3.94e-6 * s + 4.5957e-4) * s - .02609253) * s + .66047849) * s - 6.0681481) * s + 14.222222)
			  * s - 4.) * y) * x;

			bei = ((((((4.609e-5 * s - 3.79386e-3) * s + .14677204) * s - 2.3116751) * s + 11.377778) * s - 10.666667)
			  * s + .5) * x;
...
#define g(d)  ( exp(cn*(d)+th(8./x))/sqrt(two_pi()*(d)) )
	...
	if (x > 8.0) {
		/*TODO : Trying change of Pi!*/
		// nec_complex br2 = cplx_01() * f(x) / pi();
		nec_complex br2 = cplx_01() * f(x) / M_PI; // Pi();
	...
	nec_float ber = ((((((-9.01e-6 * s + 1.22552e-3) * s - .08349609) * s + 2.6419140) * s - 32.363456) * s + 113.77778)
	  * s - 64.) * s + 1.;

	nec_float bei = ((((((1.1346e-4 * s - .01103667) * s + .52185615) * s - 10.567658) * s + 72.817777) * s - 113.77778)
	  * s + 16.) * y;

	nec_complex br1 = nec_complex(ber, bei);

	ber = (((((((-3.94e-6 * s + 4.5957e-4) * s - 0.02609253) * s + 0.66047849) * s - 6.0681481) * s + 14.222222) * s -
	  4.0) * y) * x;

	bei = ((((((4.609e-5 * s - 3.79386e-3) * s + .14677204) * s - 2.3116751) * s + 11.377778) * s - 10.666667) * s +
	  .5) * x;
 ...
 */
void nec_context::gfld(nec_float rho, nec_float phi, nec_float rz, nec_complex *eth, nec_complex *epi, nec_complex *erd,
...
void nec_context::ffld(nec_float thet, nec_float phi, nec_complex *eth, nec_complex *eph, nec_float in_wavelength) {
	static nec_complex CONST3(0.0, -em::impedance() / four_pi());
	// -29.97922085;
	...
	roz = rozs; { // without ground
		complex_array temp = current_vector.segment(m_geometry->n_segments,
		  current_vector.size() - m_geometry->n_segments);
