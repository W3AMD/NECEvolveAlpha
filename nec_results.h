...
enum RESULT_FORMAT {
	RESULT_FORMAT_NEC = 1, RESULT_FORMAT_XML = 2, RESULT_FORMAT_CSV = 3
};
...
enum nec_result_type {
	RESULT_NORMALIZED_RECEIVING_PATTERN = 1, RESULT_STRUCTURE_EXCITATION = 2,
	RESULT_ANTENNA_INPUT = 3, RESULT_RADIATION_PATTERN = 4,
	RESULT_NEAR_FIELD_PATTERN = 5, RESULT_STRUCTURE_CURRENTS = 6
};
...
public:
	...
	nec_base_result() : _write_file(true), _result_format(RESULT_FORMAT_NEC) {
	}
...
public:
	nec_norm_rx_pattern(int in_n_theta, int in_n_phi, real_matrix& in_mag,
		nec_float theta0, nec_float theta_step, nec_float phi0,
		nec_float phi_step, nec_float in_eta, nec_float in_axial_ratio,
		int in_segment_number, string in_type) {
		n_theta = in_n_theta;
		n_phi = in_n_phi;
	...
	virtual void write_to_file(ostream& os) {
		...
		os << "                      NORMALIZATION FACTOR: ";
		oh.real_out(11, 4, norm_factor);
		os << endl;
		os << "                      ETA: ";
		oh.real_out(7, 2, _eta, false);
		os << " DEGREES" << endl;
		os << "                      TYPE: " << _type << endl;
		os << "                      AXIAL RATIO: ";
		oh.real_out(6, 3, _axial_ratio, false);
		os << endl;
		os << "                      SEGMENT NO: ";
		oh.int_out(5, _segment_number);
		os << endl << endl;
		os << "                      THETA     PHI       ---- PATTERN ----"
			<< endl;
		os << "                      (DEG)    (DEG)       DB     MAGNITUDE"
			<< endl;
		...
		for (int t = 0; t < n_theta; t++) {
			...
			for (int p = 0; p < n_phi; p++) {
				...
				oh.start_record();
				oh.padding("                    ");
				oh.real_out(7, 2, theta, false);
				oh.separator();
				oh.real_out(7, 2, phi, false);
				oh.separator();
				oh.padding("  ");
				oh.real_out(7, 2, gain, false);
				oh.separator();
				oh.padding("  ");
...
public:
	structure_excitation_data(int segment_number, int segment_tag,
		nec_complex voltage, nec_complex current, nec_float power) {
	...
	void write(output_helper& oh) {
		 ...
		 */
		oh.start_record();
		oh.int_out(4, m_segment_tag);
		oh.separator();
		oh.int_out(5, m_segment_number);
		oh.separator();
		oh.complex_out(11, 4, m_voltage);
		oh.separator();
		oh.complex_out(11, 4, m_current);
		oh.separator();
		oh.complex_out(11, 4, impedance);
		oh.separator();
		oh.complex_out(11, 4, admittance);
		oh.separator();
...
public:
	...
	void add(int segment, int tag, nec_complex voltage, nec_complex current,
		nec_float power) {
		n_items++;
		_tag.push_back(tag);
	...
	virtual void write_to_file(ostream& os) {
		...
		os << "  TAG   SEG       VOLTAGE (VOLTS)          CURRENT (AMPS)         IMPEDANCE (OHMS)       ADMITTANCE (MHOS)     POWER"
			<< endl;
		os << "  NO:   NO:     REAL      IMAGINARY     REAL      IMAGINARY     REAL      IMAGINARY     REAL      IMAGINARY   (WATTS)"
			<< endl;
		...
		for (int i = 0; i < n_items; i++) {
			...
			oh.int_out(4, _tag[i]);
			oh.separator();
			oh.int_out(5, _segment[i]);
			oh.separator();
			oh.complex_out(11, 4, _voltage[i]);
			oh.separator();
			oh.complex_out(11, 4, _current[i]);
			oh.separator();
			oh.complex_out(11, 4, _impedance[i]);
			oh.separator();
			oh.complex_out(11, 4, _admittance[i]);
			oh.separator();
...
public:
	...
	void set_input(int tag, int segment, nec_complex voltage,
		nec_complex current, nec_complex impedance, nec_complex admittance,
		nec_float power);
	...
	virtual void write_to_file(ostream& os) {
		...
		os << "   TAG   SEG.    VOLTAGE (VOLTS)         CURRENT (AMPS)         IMPEDANCE (OHMS)        ADMITTANCE (MHOS)      POWER"
			<< endl;
		os << "   NO.   NO.    REAL        IMAG.       REAL        IMAG.       REAL        IMAG.       REAL        IMAG.     (WATTS)"
			<< endl;
		for (int i = 0; i < n_items; i++) {
			oh.start_record();
			oh.int_out(4, _tag[i]);
			oh.separator();
			oh.int_out(5, _segment[i]);
			if (_voltage[i].real() >= 0)
				oh.separator();
			oh.complex_out(11, 5, _voltage[i]);
			if (_current[i].real() >= 0)
				oh.separator();
			oh.complex_out(11, 5, _current[i]);
			if (_impedance[i].real() >= 0)
				oh.separator();
			oh.complex_out(11, 5, _impedance[i]);
			if (_admittance[i].real() >= 0)
				oh.separator();
			oh.complex_out(11, 5, _admittance[i]);
			if (_power[i] >= 0)
				oh.separator();
			oh.real_out(11, 5, _power[i]);
			oh.end_record();
		}
	}
...
public:
	...
	void set_input(nec_float x, nec_float y, nec_float z, nec_complex field_x,
		nec_complex field_y, nec_complex field_z);
	...
	virtual void write_to_file(ostream& os) {
		...
		if (nfeh != 1) {
			...
			os << "     ------- LOCATION -------     ------- EX ------    ------- EY ------    ------- EZ ------"
				<< endl;
			os << "      X         Y         Z       MAGNITUDE   PHASE    MAGNITUDE   PHASE    MAGNITUDE   PHASE"
				<< endl;
			os << "    METERS    METERS    METERS     VOLTS/M  DEGREES    VOLTS/M   DEGREES     VOLTS/M  DEGREES"
				<< endl;
		}
		else {
			oh.section_start("NEAR MAGNETIC FIELDS");
			os << "     ------- LOCATION -------     ------- HX ------    ------- HY ------    ------- HZ ------"
				<< endl;
			os << "      X         Y         Z       MAGNITUDE   PHASE    MAGNITUDE   PHASE    MAGNITUDE   PHASE"
				<< endl;
			os << "    METERS    METERS    METERS      AMPS/M  DEGREES      AMPS/M  DEGREES      AMPS/M  DEGREES"
				<< endl;
		}
		for (int i = 0; i < n_items; i++) {
			oh.start_record();
			oh.padding(" ");
			oh.real_out(9, 4, _x[i], false);
			oh.separator();
			oh.real_out(9, 4, _y[i], false);
			oh.separator();
			oh.real_out(9, 4, _z[i], false);
			oh.separator();
			oh.padding(" ");
			oh.real_out(11, 4, abs(_field_x[i]), true);
			oh.separator();
			oh.real_out(7, 2, arg_degrees(_field_x[i]), false);
			oh.separator();
			oh.padding(" ");
			oh.real_out(11, 4, abs(_field_y[i]), true);
			oh.separator();
			oh.real_out(7, 2, arg_degrees(_field_y[i]), false);
			oh.separator();
			oh.padding(" ");
			oh.real_out(11, 4, abs(_field_z[i]), true);
			oh.separator();
			oh.real_out(7, 2, arg_degrees(_field_z[i]), false);
			oh.separator();
			oh.end_record();
		}
	}
...
public:
	...
	~nec_results() {
		// write_to_file();
		/*TODO : Check where else results can be deleted,
		 keep getting access violation here on closing because this must
		 already be deleted somewhere else
		 */
		/*
		 for (int i = 0; i < _n; i++) {
		 if (_results[i] != NULL) {
	 ...
	 */
	nec_base_result* get_result(const long index,
	...
	nec_norm_rx_pattern* get_norm_rx_pattern(const long index) {
		return (nec_norm_rx_pattern*)get_result(index,
			RESULT_NORMALIZED_RECEIVING_PATTERN);
	}
	...
	nec_radiation_pattern* get_radiation_pattern(const long index) {
		return (nec_radiation_pattern*)get_result(index,
			RESULT_RADIATION_PATTERN);
	}
	...
	nec_structure_excitation* get_structure_excitation(const long index) {
		return (nec_structure_excitation*)get_result(index,
			RESULT_STRUCTURE_EXCITATION);
	}
	...
	nec_near_field_pattern* get_near_field_pattern(const long index) {
		return (nec_near_field_pattern*)get_result(index,
			RESULT_NEAR_FIELD_PATTERN);
	}
	...
	nec_structure_currents* get_structure_currents(const long index) {
		return (nec_structure_currents*)get_result(index,
			RESULT_STRUCTURE_CURRENTS);
	}
