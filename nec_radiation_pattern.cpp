...
nec_radiation_pattern::~nec_radiation_pattern() {
...
nec_radiation_pattern::nec_radiation_pattern(int in_n_theta, int in_n_phi,
	nec_float in_theta_start, nec_float in_phi_start, nec_float in_delta_theta,
	nec_float in_delta_phi, nec_float in_range, nec_ground& in_ground,
	int in_ifar, nec_float in_wavelength, nec_float pinr, nec_float pnlr,
	int in_rp_output_format, int in_rp_normalization, int in_rp_ipd,
	int in_rp_power_average, nec_float in_gnor, c_plot_card& in_plot_card)
...
void nec_radiation_pattern::write_to_file_aux(ostream& os) {
	if (false == m_analysis_done)
		throw new nec_exception
			("Internal Error: Radiation Pattern Analysis not done");

	static const char *hpol[4] = {"LINEAR", "RIGHT ", "LEFT  ", " "};
	static const char *gain_type[2] = {
		"    - POWER GAINS -      ", "--- DIRECTIVE GAINS ---"};
	static const char *igax[4] = {" MAJOR", " MINOR", "  VERT.", "HOR."};
	...
	if (_ifar == 1) {
		...
		os << "    ------- LOCATION -------     --- E(THETA) ---     ---- E(PHI) ----    --- E(RADIAL) ---"
			<< endl;
		os << "      RHO    PHI        Z           MAG    PHASE         MAG    PHASE        MAG     PHASE"
			<< endl;
		os << "    METERS DEGREES    METERS      VOLTS/M DEGREES      VOLTS/M DEGREES     VOLTS/M  DEGREES"
			<< endl;
	}
	else { // _ifar != 1
		// oh.section_start("RADIATION PATTERNS");
		os << "                                                - - - RADIATION PATTERNS - - -";
		os << endl << endl;
		if (m_range >= 1.0e-20) {
			...
			os << "                             RANGE: ";
			oh.real_out(13, 6, m_range);
			os << " METERS" << endl;
			os << "                             EXP(-JKR)/R: ";
			oh.real_out(12, 5, exrm);
			os << " AT PHASE: ";
			oh.real_out(7, 2, exra, false);
			os << " DEGREES" << endl;
		}
		...
		os << "  - - ANGLES - -       ";
		oh.string_out(23, gain_type[m_rp_ipd]);
		os << " - - - POLARIZATION - - -    - - - E(THETA) - - -    - - - E(PHI) - - -"
			<< endl;
		os << "  THETA     PHI      ";
		oh.string_out(5, igax[itmp1]);
		os << "  ";
		oh.string_out(5, igax[itmp2]);
		os << "    TOTAL      AXIAL     TILT   SENSE     MAGNITUDE    PHASE      MAGNITUDE    PHASE "
			<< endl;
		os << " DEGREES  DEGREES       DB      DB      DB        RATIO     DEG.              VOLTS/M    DEGREES      VOLTS/M    DEGREES"
			<< endl;
	}/*if ( _ifar == 1)*/
	...
	for (int kph = 0; kph < n_phi; kph++) {
		...
		for (int kth = 0; kth < n_theta; kth++) {
			...
			if (_ifar == 1) {
				...
				oh.separator();
				oh.real_out(9, 2, thet, false);
				oh.separator();
				oh.real_out(11, 4, abs(e_theta));
				oh.separator();
				oh.real_out(7, 2, arg_degrees(e_theta), false);
				oh.separator();
				oh.real_out(11, 4, abs(e_phi));
				oh.separator();
				oh.real_out(7, 2, arg_degrees(e_phi), false);
				oh.separator();
				oh.real_out(11, 4, abs(e_r));
				oh.separator();
				oh.real_out(7, 2, arg_degrees(e_r), false);
				oh.end_record();
			}
			else {
				nec_complex e_theta = _e_theta(kth, kph);
				...
				const char* pol_sense =
					hpol[_polarization_sense_index(kth, kph)];

				// oh.start_record();
				oh.padding(" ");
				oh.real_out(7, 2, thet, false);
				// oh.separator();
				oh.real_out(9, 2, phi, false);
				oh.separator();
				oh.separator();
				oh.separator();
				oh.separator();
				// oh.padding(" ");
				oh.real_out(7, 2, _power_gain_vert(kth, kph), false);
				oh.separator();
				oh.real_out(7, 2, _power_gain_horiz(kth, kph), false);
				oh.separator();
				oh.real_out(7, 2, _power_gain_tot(kth, kph), false);
				oh.separator();
				oh.separator();
				oh.separator();
				oh.separator();
				oh.real_out(7, 5, _polarization_axial_ratio(kth, kph), false);
				oh.separator();
				oh.separator();
				oh.real_out(7, 2, _polarization_tilt(kth, kph), false);
				oh.separator();
				oh.separator();
				oh.string_out(6, pol_sense);
				oh.separator();
				oh.separator();
				oh.separator();
				oh.separator();
				oh.real_out(9, 5, abs(e_theta));
				oh.separator();
				oh.separator();
				oh.real_out(7, 2, arg_degrees(e_theta), false);
				oh.separator();
				...
				oh.separator();
				oh.separator();
				oh.real_out(9, 5, abs(e_phi));
				oh.separator();
				oh.separator();
				oh.real_out(7, 2, arg_degrees(e_phi), false);
				oh.end_record();

				m_plot_card.plot_patterns(thet, phi, e_theta, e_phi,
					_power_gain_vert(kth, kph), _power_gain_horiz(kth, kph),
					_power_gain_tot(kth, kph));
			}/*if ( _ifar != 1)*/
	...
	if (m_rp_power_average != 0) {
		oh.section_end();
		os << endl;
		os << "   AVERAGE POWER GAIN= ";
		oh.real_out(11, 5, _average_power_gain);
		os << "       SOLID ANGLE USED IN AVERAGING=(";
		oh.real_out(7, 4, _average_power_solid_angle, false);
		os << ")*PI STERADIANS." << endl << endl;
	}
...
void nec_radiation_pattern::analyze(nec_context* m_context) {
	...
	else if (m_context->m_excitation_type == EXCITATION_CURRENT) {
		_pinr = 394.51 * m_context->xpr6 * m_context->xpr6 * _wavelength *
			_wavelength;
		gcop = _wavelength * _wavelength * 2. * pi() /
			(em::impedance() * _pinr);
		prad = _pinr - m_context->structure_power_loss - _pnlr;
		gcon = gcop;
		if (m_context->ipd != 0)
		...
		for (int kth = 0; kth < n_theta; kth++) {
			...
			if (1 == _ifar) {
				...
				if ((ethm2 <= 1.0e-20) && (ephm2 <= 1.0e-20)) {
					...
					/* TODO : is there trouble here from this atan2? */
...

				{
					...
					default:
						throw new nec_exception
							("Segment error - Unknown Gain Normalization Encountered.");
					}
				...
				if (m_rp_power_average != 0) {
					...
					nec_float da =
						fabs(delta_phi_rad * (cos(tmp3) - cos(tmp4)));
					if ((kph == 1) || (kph == n_phi))
						da *= .5;
					pint += tstor1 * da;

					if (m_rp_power_average == 2)
						// do not print the power gain values (just compute the average)
							continue;
				}
				...
				_averaging_scales(kth, kph) =
					(sin(tha) * (n_theta - 1) / n_theta) + 1.0 / n_theta;
			}/*if ( _ifar == 1)*/
		}/*for( kth = 1; kth <= n_theta; kth++ )*/
	...
	if (m_rp_power_average != 0) {
		...
		nec_float theta_range = degrees_to_rad(delta_theta) * (nec_float)
			(n_theta - 1);
		nec_float phi_range = degrees_to_rad(delta_phi) * (nec_float)
			(n_phi - 1);
		nec_float total_theta = theta_start_rad + theta_range;
		nec_float solid_angle =
			fabs(phi_range * (cos(theta_start_rad) - cos(total_theta)));

		_average_power_gain = pint / solid_angle;
		_average_power_solid_angle = solid_angle / pi();
		// We display it as a multiple of pi()
	}
...
void nec_radiation_pattern::write_normalized_gain(ostream& os) {
	...
	switch (m_rp_normalization) {
	case 1:
		norm_type = "MAJOR AXIS";
		break;
	case 2:
		norm_type = "MINOR AXIS";
		break;
	case 3:
		norm_type = "VERTICAL";
		break;
	case 4:
		norm_type = "HORIZONTAL";
		break;
	case 5:
		norm_type = "TOTAL ";
		break;
	...
	oh.section_start("NORMALIZED GAIN"); {
		stringstream mess;
		mess << norm_type << " GAIN";
		oh.center_text(mess.str(), "");
	} {
		stringstream mess;
		mess << "NORMALIZATION FACTOR: " << normalization_factor << " dB";
		oh.center_text(mess.str(), "");
	}
	/*os << "                                      " << norm_type << " GAIN" << endl;
	 os << "                                   NORMALIZATION FACTOR: "; oh.real_out(7,2,normalization_factor,false); os << " db" << endl << endl;*/
	os << "    ---- ANGLES ----                ---- ANGLES ----                ---- ANGLES ----"
		<< endl;
	os << "    THETA      PHI        GAIN      THETA      PHI        GAIN      THETA      PHI       GAIN"
		<< endl;
	os << "   DEGREES   DEGREES        DB     DEGREES   DEGREES        DB     DEGREES   DEGREES       DB"
		<< endl;
	...
	for (int p = 0; p < n_phi; p++) {
		...
		for (int t = 0; t < n_theta; t++) {
			...
			oh.real_out(9, 2, phi, false);
			oh.separator();
			oh.real_out(9, 2, norm_gain, false);
			oh.padding(" ");
