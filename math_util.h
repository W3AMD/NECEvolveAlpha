...
inline void vector_fill(complex_array& x, int32_t start, int32_t N,
	const nec_complex& y) {
	x.fill(start, N, y);
}

inline nec_complex cplx_00() {
	static nec_complex _cplx00(0.0, 0.0);
	return _cplx00;
}

inline nec_complex cplx_01() {
	static nec_complex _cplx01(0.0, 1.0);
	return _cplx01;
}

inline nec_complex cplx_10() {
	static nec_complex _cplx10(1.0, 0.0);
	return _cplx10;
}

inline nec_complex cplx_11() {
	static nec_complex _cplx11(1.0, 1.0);
	return _cplx11;
}
...
#define M_PI_SQR 1.7724538509055160272981674833411
inline nec_float sqrt_pi() { // was SP from common.h
	//static nec_float _tmp = sqrt(M_PI);
	return M_PI_SQR;
}

inline nec_complex two_pi_j() {
	//static nec_complex _tmp(0.0, 2.0 * M_PI);
	return {0.0, 2.0 * M_PI};
}

inline nec_float rad_to_degrees(nec_float in_radians) {
	static nec_float _rad_to_deg = 360.0 / (2.0 * M_PI); // 57.29577951
	return in_radians * _rad_to_deg;
}

inline nec_float degrees_to_rad(nec_float in_degrees) {
	static nec_float _deg_to_rad = (2.0 * M_PI) / 360.0;
...
public:
	...
	nec_3vector(const nec_float& in_x, const nec_float& in_y,
	...
	nec_3vector operator*(const nec_3vector& a) const {
		return nec_3vector(_v[1]*a._v[2] - _v[2]*a._v[1],
			_v[2]*a._v[0] - _v[0]*a._v[2], _v[0]*a._v[1] - _v[1]*a._v[0]);
	}

	inline nec_float x() const {
		return _v[0];
	}

	inline nec_float y() const {
		return _v[1];
	}

	inline nec_float z() const {
		return _v[2];
	}
