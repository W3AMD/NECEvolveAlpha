 ...
 */
void nec_ground::parse_gn(int itmp1, int itmp2, nec_float tmp1, nec_float tmp2,
 ...
 */
void nec_ground::setup_cliff(nec_float in_eprs2, nec_float in_sig2,
...
nec_complex nec_ground::get_zrati2(nec_float _wavelength) {
	return sqrt(1.0 / nec_complex(epsr2,
		-sig2 * _wavelength * em::impedance_over_2pi()));
}

void nec_ground::write_to_file_aux(std::ostream& os, output_helper& oh,
	int _ifar) {
	if (_ifar >= 2) {
		...
		if (_ifar > 3) {
			os << endl;
			os << "                                        RADIAL WIRE GROUND SCREEN"
				<< endl;
			os << "                                        ";
			oh.int_out(5, radial_wire_count);
			os << " WIRES" << endl;
			os << "                                        WIRE LENGTH= ";
			oh.real_out(8, 2, radial_wire_length, false);
			os << " METERS" << endl;
			os << "                                        WIRE RADIUS= ";
			oh.real_out(10, 3, radial_wire_radius);
			os << " METERS" << endl;
		}/*if ( _ifar > 3)*/
		...
		if (_ifar != 4) {
			...
			os << "                                        " << hclif <<
				" CLIFF" << endl;
			os << "                                        EDGE DISTANCE= ";
			oh.real_out(9, 2, cliff_edge_distance, false);
			os << " METERS" << endl;
			os << "                                        HEIGHT= ";
			oh.real_out(8, 2, cliff_height, false);
			os << " METERS" << endl;
			os << "                                        SECOND MEDIUM -"
				<< endl;
			os << "                                        RELATIVE DIELECTRIC CONST.= ";
			oh.real_out(7, 3, epsr2, false);
			os << endl;
			os << "                                        CONDUCTIVITY= ";
			oh.real_out(10, 3, sig2, false);
			os << " MHOS" << endl;
		}/*if ( _ifar != 4 )*/
	}/*if ( _ifar >= 2)*/
}

void nec_ground::calculate_antenna_environment(c_ground_wave& ground_wave,
	nec_float freq_mhz) {
	if (false == present()) {
		return;
	}
	...
	ground_wave.set_u(zrati); // 0 ms
	unsigned int end = GetTickCount();
	unsigned int groundwavesettime = end - start;
	...
	if (type_sommerfeld_norton()) {
		...
		start = GetTickCount();
		/*TODO :
		 if the wavelength is in the same band here there's no need to calculate
		 the same grid*/
		/*TODO :
		 On a frequency sweep file the sommerfeld is calculated every time
		 Does it really make that much difference if we are within 1 wavelength?*/
		if (wavelength!=previousWavelength) {
			_ggrid.sommerfeld(epsr, sig, wavelength); // 969 ms
			// set this as the wavelength so we know we don't
			// have to calculate it again at this wavelength
			previousWavelength = wavelength;
		}
		end = GetTickCount();
		unsigned int sommerfeldtime = end - start;
		...
		if (abs((_ggrid.m_epscf - _epsc) / _epsc) >= 1.0e-3) {
			nec_stop("ERROR IN GROUND PARAMETERS -"
				...
				, real(_ggrid.m_epscf), imag(_ggrid.m_epscf), real(_epsc),
				imag(_epsc));
		}
	}
}
