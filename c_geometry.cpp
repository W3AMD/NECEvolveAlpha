...
#include <cstring>
#include <stdint.h>
#include "nec_globals.h"

c_geometry::c_geometry() : patch_x1(0, 0, 0), patch_x2(0, 0, 0),
	patch_x3(0, 0, 0), patch_x4(0, 0, 0) {
	n_segments = 0;
...
void c_geometry::parse_geometry(nec_context* in_context, FILE* input_fp) {
	...
	int card_int_1, card_int_2;
	/*The two integer parameters from the geometry card*/
	...
	do {
		read_geometry_card(input_fp, gm, &card_int_1, &card_int_2, &xw1, &yw1,
			&zw1, &xw2, &yw2, &zw2, &rad);
		...
		if (card_id == "GW") {
			...
			m_output->nec_printf
				("%6d %10.5f %10.5f %10.5f  %10.5f"
				" %10.5f %10.5f %10.5f %6d %8d %5d %7d", nwire, xw1, yw1, zw1,
				xw2, yw2, zw2, rad, wire_segment_count, n_segments + 1,
				n_segments + wire_segment_count, wire_tag);
			m_output->endl(1);
			...
			wire(wire_tag, wire_segment_count, xw1, yw1, zw1, xw2, yw2, zw2,
				rad, xs1, ys1);
		}
		...
		/*original structure in new positions. */
			...
			move(xw1, yw1, zw1, xw2, yw2, zw2, (int)(rad + .5), card_int_2,
				card_int_1);
		}
		...
		/*"sp" card, generate single new patch*/
			...
			m_output->nec_printf
				("\n" " %5d%c %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f", i1,
				ipt[card_int_2 - 1], xw1, yw1, zw1, xw2, yw2, zw2);
			...
			if (card_int_2 > 1) {
				...
				int ix, iy;
				read_geometry_card(input_fp, gm, &ix, &iy, &x3, &y3, &z3, &x4,
					&y4, &z4, &dummy);
				...
				m_output->nec_printf
					("\n" "      %11.5f %11.5f %11.5f %11.5f %11.5f %11.5f", x3,
					y3, z3, x4, y4, z4);
			...
			patch(card_int_1, card_int_2, xw1, yw1, zw1, xw2, yw2, zw2, x3, y3,
				z3, x4, y4, z4);
		}
		...
		/*"sm" card, generate multiple-patch surface*/
			...
			m_output->nec_printf
				...
				"     SURFACE - %d BY %d PATCHES", i1, ipt[1], xw1, yw1, zw1,
				xw2, yw2, zw2, card_int_1, card_int_2);
			...
			int ix, iy;
			read_geometry_card(input_fp, gm, &ix, &iy, &x3, &y3, &z3, &x4, &y4,
				&z4, &dummy);
			...
			m_output->nec_printf
				("\n" "      %11.5f %11.5f %11.5f %11.5f %11.5f %11.5f", x3, y3,
				z3, x4, y4, z4);
		...
		/*"ga" card, generate segment data for wire arc*/
			...
			m_output->nec_printf
				...
				"       %11.5f %5d %5d %5d %4d", nwire, xw1, yw1, zw1, xw2,
				card_int_2, i1, i2, card_int_1);
		...
		/*"sc" card*/
			...
			m_output->nec_printf
				("\n" " %5d%c %10.5f %11.5f %11.5f %11.5f %11.5f %11.5f", i1,
				ipt[card_int_2 - 1], xw1, yw1, zw1, xw2, yw2, zw2);
		...
		/*"gh" card, generate helix*/
			...
			m_output->nec_printf
				...
				, nwire, xw1, yw1, rad, card_int_2, i1, i2, card_int_1, zw1,
				xw2, yw2, zw2);
...
void c_geometry::geometry_complete(nec_context* in_context, int gpflag) {
	...
	for (uint32_t i = 0; i < m_wires.size(); i++) {
		...
		for (uint32_t j = 0; j < m_wires.size(); j++) {
			if (i > j) {
				...
				if (wires.size() > 2) {
					nec_exception nex =
						nec_exception("GEOMETRY DATA ERROR -- WIRE # ");
					nex.append(j + 1);
					nex.append(" (TAG ID #");
					nex.append(b.tag_id());
	...
	if (n_segments != 0) {
		...
		XCenterSeg.resize(n_segments);
		YCenterSeg.resize(n_segments);
		ZCenterSeg.resize(n_segments);
		...
		m_output->nec_printf
			("\n" "  SEG.   COORDINATES OF SEG. CENTER     SEG.     ORIENTATION"
		...
		for (int i = 0; i < n_segments; i++) {
			...
			// store the orignal center position before overwriting it
			std::vector<nec_float> *test = &XCenterSeg;
			...
			m_output->nec_printf
				...
				"  %9.5f %9.5f %9.5f %5d %4d %4d %6d", i + 1, x[i], y[i], z[i],
				segment_length[i], xw2, yw2, segment_radius[i], icon1[i], i + 1,
				icon2[i], segment_tags[i]);

			in_context->plot_card.plot_segments(i, x, y, z, segment_length, xw2,
				yw2, segment_radius, icon1, icon2);
	...
	if (m != 0) {
		...
		for (int i = 0; i < m; i++) {
			...
			m_output->nec_printf
				...
				" %10.5f  %8.4f %8.4f %8.4f  %8.4f %8.4f %8.4f", i + 1, px[i],
				py[i], pz[i], xw1, yw1, zw1, pbi[i], t1x[i], t1y[i], t1z[i],
				t2x[i], t2y[i], t2z[i]);
		}/*for( i = 0; i < m; i++ )*/
	}/*if ( m == 0)*/
...
void c_geometry::get_xyzSegmentCoord(int index, nec_float* segX,
...
void c_geometry::get_xyzSegmentOrignalAbsolute(int index, nec_float* segX,
	nec_float* segY, nec_float* segZ) {
	// check the size
	int size = XCenterSeg.size();
	*segX = XCenterSeg[index];
	*segY = YCenterSeg[index];
	*segZ = ZCenterSeg[index];
	return;
}
 ...
 */
void c_geometry::wire(int tag_id, int segment_count, nec_float xw1,
	nec_float yw1, nec_float zw1, nec_float xw2, nec_float yw2, nec_float zw2,
	...
	int wiressize = m_wires.size();
	for (uint32_t i = 0; i < m_wires.size(); i++) {
		nec_wire a = m_wires[i];
		if (a.intersect(seg_midpoint)) {
			/*TODO : why would we get here on the second pass with the same file?*/
			/*TODO : When this exception is thrown the file is not closed and
 ...
 */
void c_geometry::helix(int tag_id, int segment_count, nec_float s, nec_float hl,
	...
	for (int i = ist; i < n_segments; i++) {
		...
		if (a2 == a1) {
			...
			x[i] = (a1 + (a2 - a1) * z[i] / fabs(hl)) * cos
				(2. * pi() * z[i] / s);
			y[i] = (b1 + (b2 - b1) * z[i] / fabs(hl)) * sin
				(2. * pi() * z[i] / s);
			x2[i] = (a1 + (a2 - a1) * z2[i] / fabs(hl)) * cos
				(2. * pi() * z2[i] / s);
			y2[i] = (b1 + (b2 - b1) * z2[i] / fabs(hl)) * sin
				(2. * pi() * z2[i] / s);
		}/*if ( a2 == a1)*/
	...
	if (a2 != a1) {
		...
		m_output->nec_printf("\n       THE CONE ANGLE OF THE SPIRAL IS %10.4f",
			sangle);
		return;
	}
		...
		if (iz != 0) {
			...
			if (n_segments > 0) {
				...
				for (i = 0; i < n_segments; i++) {
					...
					{
						nec_exception* nex =
							new nec_exception("GEOMETRY DATA ERROR--SEGMENT ");
						nex->append(i + 1);
			...
			if (m > 0) {
				...
				for (i = 0; i < m; i++) {
					...
					if (fabs(pz[i]) <= 1.0e-10) {
						nec_exception* nex =
							new nec_exception("GEOMETRY DATA ERROR--PATCH ");
						nex->append(i + 1);
						nex->append("LIES IN PLANE OF SYMMETRY");
						throw nex;
					}
...
void c_geometry::connect_segments(int ignd) {
	...
	if (ignd != 0) {
		...
		if (np > n_segments) {
			throw new nec_exception
				("ERROR: NP > N IN c_geometry::connect_segments()");
		}
	...
	m_output->nec_printf
		("\n" "   TOTAL SEGMENTS USED=  %d     NO. SEG. IN A SYMMETRIC CELL="
		"  %d     SYMMETRY FLAG=  %d", n_segments, np, m_ipsym);
	...
	if (0 == np + mp)
		throw new nec_exception
			("connect_segments Geometry has zero wires and zero patches.");
	...
	// used just to print the junction number out if there are 3 or more segments
		...
		while (true) {
			if ((ix != 0) && (ix != (j + 1)) && (ix <= PCHCON)) {
				...
				do {
					...
					if (jend != 1) {
						xa = xa + x[ixx];
						// dies here if n_segments == 1. ix is totally fried.
						ya = ya + y[ixx];
						za = za + z[ixx];
						ix = icon1[ixx];
					}
...
void c_geometry::sp_card(int ns, nec_float in_x1, nec_float in_y1,
	nec_float in_z1, nec_float in_x2, nec_float in_y2, nec_float in_z2) {
	const char ipt[4] = {'P', 'R', 'T', 'Q'};
	this->patch_type = ns;
	m_output->nec_printf
		("\n" " %5d%c %10.5f %10.5f %10.5f %10.5f %10.5f %10.5f", m + 1,
		ipt[ns], in_x1, in_y1, in_z1, in_x2, in_y2, in_z2);
	switch (ns) {
	case 0: { // Arbitrary Shape
			nec_float elevation = degrees_to_rad(in_x2);
			nec_float azimuth = degrees_to_rad(in_y2);
			patch(0, 0, in_x1, in_y1, in_z1, elevation, azimuth, in_z2, 0, 0, 0,
				0, 0, 0);
			break;
		}
 ...
 */
void c_geometry::sc_card(int i2, nec_float x3, nec_float y3, nec_float z3,
	...
	case 0: // Arbritrary
		throw new nec_exception
			("PATCH DATA ERROR: SC CARD FOR ARBITRARY PATCH TYPE (NS=0) ");
		break;
	...
	patch(this->patch_type, i2, this->patch_x1(0), this->patch_x1(1),
		this->patch_x1(2), this->patch_x2(0), this->patch_x2(1),
		this->patch_x2(2), this->patch_x3(0), this->patch_x3(1),
		this->patch_x3(2), this->patch_x4(0), this->patch_x4(1),
		this->patch_x4(2));
 ...
 * shapes are not allowed in a string of linked patches.
 */
void c_geometry::sc_multiple_card(int i2, nec_float x3, nec_float y3,
	nec_float z3, nec_float x4, nec_float y4, nec_float z4) {
	...
	case 0: // Arbritrary
		throw new nec_exception
			("PATCH DATA ERROR: MULTIPLE SC CARDS FOR ARBITRARY PATCH TYPE (NS=0) "
			);
		break;
	case 2: // Triangular
		throw new nec_exception
			("PATCH DATA ERROR: MULTIPLE SC CARDS FOR TRIANGULAR PATCH (NS=2) "
			);
		break;
	...
	m_output->nec_printf
		("\n" " %5d%c %10.5f %11.5f %11.5f %11.5f %11.5f %11.5f",
		this->patch_type, ipt[i2], this->patch_x1(0), this->patch_x1(1),
		this->patch_x1(2), this->patch_x2(0), this->patch_x2(1),
		this->patch_x2(2));

	m_output->nec_printf
		("\n" "      %11.5f %11.5f %11.5f  %11.5f %11.5f %11.5f", x3, y3, z3,
		x4, y4, z4);
 ...
 */
void c_geometry::patch(int nx, int ny, nec_float ax1, nec_float ay1,
	nec_float az1, nec_float ax2, nec_float ay2, nec_float az2, nec_float ax3,
	nec_float ay3, nec_float az3, nec_float ax4, nec_float ay4, nec_float az4) {
	int mi, ntp, iy, ix;
	nec_float s1x = 0., s1y = 0., s1z = 0., s2x = 0., s2y = 0., s2z = 0.,
		xst = 0.;
	...
	if (ntp <= 1) {
		...
		if (ntp <= 2) {
			...
			if (ntp != 4) {
				...
				salpn = 1. / (3. * (xa + xst));
				px[mi] = (xa * (ax1 + ax2 + ax3) + xst * (ax1 + ax3 + ax4))
					* salpn;
				py[mi] = (xa * (ay1 + ay2 + ay3) + xst * (ay1 + ay3 + ay4))
					* salpn;
				pz[mi] = (xa * (az1 + az2 + az3) + xst * (az1 + az3 + az4))
					* salpn;
				pbi[mi] = .5 * (xa + xst);
...
void c_geometry::read_geometry_card(FILE* input_fp, char *gm, int *in_i1,
	int *in_i2, nec_float *in_x1, nec_float *in_y1, nec_float *in_z1,
	nec_float *in_x2, nec_float *in_y2, nec_float *in_z2, nec_float *in_rad) {
	char line_buf[134];
	int i, line_idx;
	...
	/*check for tabs and replace for compatibility with 4NEC2*/
	for (int loopTabs = 0; loopTabs < line_length; loopTabs++) {
		if (line_buf[loopTabs] == '\t') {
			line_buf[loopTabs] = ',';
		}
	}
	...
	if (strcmp(gm, "XT") == 0) {
		nec_exception* nex =
			new nec_exception
			("Exiting after an \"XT\" command in read_geometry_card()");
		throw nex;
	}
	...
	for (i = 0; i < n_integer_params; i++) {
		...
		/*TODO : Not sure about the changes added tab character and period*/
		line_idx--;
		while ((line_buf[++line_idx] != ' ') && (line_buf[line_idx] != ',') &&
			...
			if (((line_buf[line_idx] < '0') || (line_buf[line_idx] > '9')) &&
				(line_buf[line_idx] != '+') && (line_buf[line_idx] != '-')) {
				nec_stop("GEOMETRY DATA CARD \"%s\" ERROR:"
			...
			if (jcox != i) {
				...
				if (abs(jcox) != i) {
					if (jcox != 0)
						...
						nec_exception* nex =
							new nec_exception
							("TBF - SEGMENT CONNECTION ERROR FOR SEGMENT ");
						nex->append(i);
						throw nex;
					}
				}
	...
	do {
		...
		if (jcox != j) {
			...
			if (jcox == 0) {
				nec_exception* nex =
					new nec_exception
					("TRIO - SEGMENT CONNENTION ERROR FOR SEGMENT ");
				nex->append(j);
				throw nex;
			}
			else
...
{
	int local_jsno;
	// this parameter is renamed because it shadows the member variable of the same name
			...
			if (jcox != i) {
				...
				if (abs(jcox) != i) {
					if (jcox == 0) {
						nec_exception* nex =
							new nec_exception
							("SBF - SEGMENT CONNECTION ERROR FOR SEGMENT ");
						nex->append(i);
						throw nex;
					}
					else
 ...
 */
void c_geometry::get_current_coefficients(nec_float wavelength,
	complex_array& curx, real_array& air, real_array& aii, real_array& bir,
	real_array& bii, real_array& cir, real_array& cii, complex_array& vqds,
	int nqds, int_array& iqds) {
	...
	if (n_segments != 0) {
		...
		for (int is = 0; is < nqds; is++) {
			...
			nec_complex curd =
				s_CCJ * vqds[is] / ((log(2. * sh / segment_radius[i]) - 1.) *
				(bx[jsno - 1] * cos(two_pi() * sh) + cx[jsno - 1] * sin
				(two_pi() * sh)) * wavelength);
			ar = real(curd);
			ai = imag(curd);
...
void c_geometry::frequency_scale(nec_float freq_mhz) {
	/*TODO :
	 The segment length check does not seem to include frequency sweeps
	...
	for (int i = 0; i < n_segments; i++) {
		...
		double calcedSegLen = segment_length[i];
		if (segment_length[i] < 0.02) {
			/*TODO : Add exception for warning here*/
			/*m_output->nec_printf
			 ("WARNING - SEGMENT[%i] LENGTH TOO SMALL (%f)\n", i,
			 segment_length[i]);*/
...
void c_geometry::fflds(nec_float rox, nec_float roy, nec_float roz,
	complex_array& scur, nec_complex *in_ex, nec_complex *in_ey,
	nec_complex *in_ez) {
...
int c_geometry::test_ek_approximation(int seg1, int seg2) {
	...
	nec_float xi = fabs(cab[seg1] * cab[seg2] + sab[seg1] * sab[seg2] +
		salp[seg1] * salp[seg2]);
...
nec_float c_geometry::patch_angle(int patch_index, nec_float in_ax,
	nec_float in_ay, nec_float in_az) {
	return two_pi()*(in_ax*px[patch_index] + in_ay*py[patch_index] +
		in_az*pz[patch_index]);
}

void c_geometry::clear_wires(void) {
	m_wires.clear();
}
