...
int nec_main(int argc, char **argv, nec_output_file& s_output,
	nec_context* s_context, FILE* input_fp, FILE* output_fp);
...
int mainforNEC(int argc, char **argv, nec_context* s_context, FILE* input_fp,
	...
	/*try {*/
	nec_main(argc, argv, s_output, s_context,input_fp,output_fp);
	/*}
...
int readmn(FILE* input_fp, FILE* output_fp, char *gm, int *i1, int *i2, int *i3,
	int *i4, nec_float *f1, nec_float *f2, nec_float *f3, nec_float *f4,
	nec_float *f5, nec_float *f6);
...
#define CMD_NUM  21
 ...
 "EN", "WG", "MP"};
	...
	// remove s_context and move it to the outside
	// nec_context s_context;
	optind = 0;
	/*process command line options*/
	/*DONE :
	 Need to figure out why the second time into this function
	 This filename isn't picked up*/
...
#ifdef _MSC_VER
			cout << ("nec2++ " nec_version)
				<< " compiler: " << _MSC_VER << endl;
#else
			cout << ("nec2++ " nec_version) << (" compiler: " __VERSION__)
				<< endl;
#endif
			exit(0);
	...
	if (output_filename == "") {
		...
		output_filename = input_filename.substr(0, input_filename.find(".", 0))
			+ ".out";
	}
	...
	//if this is the second time through we still have many things
	//in the vectors, we will clear them here
	//the intention is to keep the sommerfield ground matrix so it doesn't
	//have to be recalculated if the frequency hasn't changed (typical)

	//clear the wire vector
	c_geometry* geo = s_context->get_geometry();
	geo->clear_wires();
	//clear the results vector
    s_context->m_results.clear_results();
	...
	while (true) {
		...
		s_output.line("1");
		s_output.endl(3);
		s_output.set_indent(32);
		s_output.line(" *********************************************");
		s_output.endl(1);
		s_output.line("    NUMERICAL ELECTROMAGNETICS CODE (NECEVOLVE)");
		s_output.endl(1);
		s_output.line(" *********************************************");
		...
		if ((strcmp(ain, "CM") == 0) || (strcmp(ain, "CE") == 0)) {
			s_output.end_section();
			s_output.set_indent(37);
			s_output.line("- - - - COMMENTS - - - -");
			s_output.endl(2);
			s_output.set_indent(25);
			char insert[255];
			int result;
			while (strcmp(ain, "CM") == 0) {
				...
				if (load_line(line_buf, input_fp) == EOF)
					throw nec_exception
						("Error reading input file (comments not terminated?)");

				/*separate card's id mnemonic*/
				strncpy(ain, line_buf, 2);
				ain[2] = '\0';

				result = sprintf(insert, "%100s", &line_buf[3]);
				/*write comment to output file*/
				s_output.line(insert);
			}
			insert[0] = '\0';
			// pad a line for 4NEC2
			result = sprintf(insert, "%52s", &insert[0]);
			s_output.line(insert);
		...
		/* TODO : trouble here the second time through with the same s_context structure */
		c_geometry* geo = s_context->get_geometry();
		...
		while (!next_job) {
			...
			int parameter_count = readmn(input_fp, output_fp, ain, &itmp1,
				&itmp2, &itmp3, &itmp4, &tmp1, &tmp2, &tmp3, &tmp4,
				&tmp5, &tmp6);
			...
			if (strcmp(ain, "XT") == 0) {
				...
				s_output.endl();
				s_output.line
					("nec2++: Exiting after an \"XT\" command in main()");
				exit(0);
			}

			data_card_count++;
			fprintf(output_fp,
				"\n ***** DATA CARD NO. %2d   " "%s %3d %5d %5d %5d %12.5E %12.5E %12.5E %12.5E %12.5E %12.5E",
				data_card_count, ain, itmp1, itmp2, itmp3, itmp4, tmp1, tmp2,
				tmp3, tmp4, tmp5, tmp6);
			...
			case 1: /*"ld" card, loading parameters*/
				s_context->ld_card(itmp1, itmp2, itmp3, itmp4, tmp1,
					tmp2, tmp3);
				continue;

			case 2: /*"gn" card, ground parameters under the antenna*/
				s_context->gn_card(itmp1, itmp2, tmp1, tmp2, tmp3, tmp4,
					tmp5, tmp6);
				continue;

			case 3: /*"ex" card, excitation parameters*/
				s_context->ex_card((enum excitation_type)itmp1, itmp2, itmp3,
					itmp4, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6);
				continue; /*continue card input loop*/

			case 4: /*"nt" card, network parameters*/
				s_context->nt_card(itmp1, itmp2, itmp3, itmp4, tmp1, tmp2, tmp3,
					tmp4, tmp5, tmp6);
				continue; /*continue card input loop*/
			...
			case 5: /*"tl" card, network parameters*/
				if (parameter_count < 10) {
					...
					s_output.endl();
					s_output.line
						("nec2++: Missing parameters in \"TL\" card. Blank parameters should be specified as zero."
						);
					exit(0);
				}
				s_context->tl_card(itmp1, itmp2, itmp3, itmp4, tmp1, tmp2, tmp3,
					tmp4, tmp5, tmp6);
				continue; /*continue card input loop*/
			...
			case 8: /*"rp" card, standard observation angle parameters*/ {
					...
					s_context->rp_card(itmp1, itmp2, itmp3, X, N, D, A, tmp1,
			...
			case 12: /*"ne" card, near field calculation parameters*/
				s_context->ne_card(itmp1, itmp2, itmp3, itmp4, tmp1, tmp2, tmp3,
					tmp4, tmp5, tmp6);
				continue;

			case 13: /*"nh" card, near field calculation parameters*/
				s_context->nh_card(itmp1, itmp2, itmp3, itmp4, tmp1, tmp2, tmp3,
					tmp4, tmp5, tmp6);
				continue;
			...
			case 17: /*"pl" card, plot flags*/ {
					...
					try {
						s_context->pl_card(ploutput_filename.c_str(), itmp1,
							itmp2, itmp3, itmp4);
					}
					catch (...) {
						char mesg[88] = "nec2++: ";
			...
			default:
				...
				secnds(&tmp1);
				tmp1 -= ex_timer;
				fprintf(output_fp, "\n\n RUN TIME= %1.3f", (float)tmp1*0.001);
