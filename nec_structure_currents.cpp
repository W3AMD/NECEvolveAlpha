...
nec_structure_currents::nec_structure_currents(nec_context * in_context,
	enum excitation_type in_pattype, int in_nload, nec_float in_xpr3,
	nec_float in_xpr6) {
...
std::string nec_structure_currents::hpol(enum excitation_type e) {
	switch (e) {
	case EXCITATION_LINEAR:
		return std::string("LINEAR");
	case EXCITATION_CIRC_RIGHT:
		return std::string("RIGHT");
	case EXCITATION_CIRC_LEFT:
...
void nec_structure_currents::analyze() {
	...
	if (m_geometry->n_segments != 0) {
		if (iptflg != -1) {
			...
			for (int i = 0; i < m_geometry->n_segments; i++) {
				...
				if ((nload != 0) && (fabs(real(m_context->zarray[i])) >=
					1.e-20))
					structure_power_loss +=
						0.5 * cmag * cmag * real(m_context->zarray[i])
						* m_geometry->segment_length[i];
				...
				if (iptflg > -1) // I
				{
					...
					if (iptflg != 0) {
						if (iptflg >= 2) {
							m_context->fnorm(m_context->get_inc() - 1, 0)
								= cmag; // WARNING: work out what inc, isave are.
					...
					else /*iptflg == 0, only the currents specified will be printed, using the standard format*/
					{
						...
						current_nb_elements++;
						_current_segment_number.push_back(i + 1);
						_current_segment_tag.push_back
							(m_geometry->segment_tags[i]);
						...
						_current_segment_length.push_back
							(m_geometry->segment_length[i]);
						_current.push_back(curi);

						// added test for plot_card.is_valid()
						if (m_context->plot_card.is_valid()
							&& m_context->plot_card.currents()) {
		...
		if (iptflq != -1) {
			...
			for (int i = 0; i < m_geometry->n_segments; i++) {
				...
				nec_complex curi = fr * nec_complex(-m_context->bii[i],
					m_context->bir[i]);
				...
				_q_density_segment_length.push_back
					(m_geometry->segment_length[i]);
	...
	if (m_geometry->m != 0) {
		...
		for (int i = 0; i < m_geometry->m; i++) {
			...
			eth = ex * m_geometry->t1x[itmp1] + ey * m_geometry->t1y[itmp1] +
				ez * m_geometry->t1z[itmp1];
			eph = ex * m_geometry->t2x[itmp1] + ey * m_geometry->t2y[itmp1] +
				ez * m_geometry->t2z[itmp1];
...
void nec_structure_currents::write_to_file_aux(ostream& os) {
	...
	if (m_geometry->n_segments != 0) {
		if (iptflg != -1) {
			if (iptflg <= 0) {
				os << "                             - - - CURRENTS AND LOCATION - - -";
				os << endl << endl;
				//oh.section_start("CURRENTS AND LOCATION");
				//oh.center_text("DISTANCES IN WAVELENGTHS", "");
				os << "                                 DISTANCES IN WAVELENGTHS";
				os << endl << endl << endl;
				os << "  SEG.  TAG    COORD. OF SEG. CENTER     SEG.            - - - CURRENT (AMPS) - - -"
					<< endl;
				os << "  NO.   NO.     X        Y        Z      LENGTH     REAL        IMAG.       MAG.        PHASE"
					<< endl;

				for (int i = 0; i < current_nb_elements; i++) {
					//oh.start_record();
					//oh.padding(" ");
					oh.int_out(6, _current_segment_number[i]);
					oh.separator();
					oh.int_out(4, _current_segment_tag[i]);
					oh.separator();
					oh.real_out(8, 4, _current_segment_center_x[i], false);
					oh.separator();
					oh.real_out(8, 4, _current_segment_center_y[i], false);
					oh.separator();
					oh.real_out(8, 4, _current_segment_center_z[i], false);
					oh.separator();
					oh.real_out(8, 5, _current_segment_length[i], false);
					oh.separator();
					oh.separator();
					oh.real_out(11, 4, real(_current[i]), true);
					oh.separator();
					oh.real_out(11, 4, imag(_current[i]), true);
					oh.separator();
					oh.real_out(11, 4, abs(_current[i]), true);
					oh.separator();
					oh.real_out(8, 3, arg_degrees(_current[i]), false);
					oh.end_record();
				}
			}
			else if (iptflg != 3) {
				if (m_context->get_inc() <= 1) {
					...
					os << "                      ETA: ";
					oh.real_out(7, 2, xpr3, false);
					os << " DEGREES" << endl;
					os << "                      TYPE: ";
					oh.string_out(6, nec_structure_currents::hpol(pattype));
					os << endl;
					os << "                      AXIAL RATIO: ";
					oh.real_out(6, 3, xpr6, false);
					os << endl << endl;
					os << "            THETA     PHI      ----- CURRENT ----    SEG"
						<< endl;
					os << "            (DEG)    (DEG)     MAGNITUDE    PHASE    NO:"
						<< endl;
				...
				oh.start_record();
				oh.padding("          ");
				oh.real_out(7, 2, _current_theta[i], false);
				oh.separator();
				oh.real_out(7, 2, _current_phi[i], false);
				oh.separator();
				oh.padding("   ");
				oh.real_out(11, 4, abs(_current[i]), true);
				oh.separator();
				oh.padding(" ");
				oh.real_out(7, 2, arg_degrees(_current[i]), false);
				oh.separator();
				oh.padding(" ");
				oh.int_out(5, _current_segment_number[i]);
				oh.separator();
				oh.end_record();
		...
		if (iptflq != -1) {
			...
			oh.center_text("DISTANCES IN WAVELENGTHS", "");
			os << endl;
			os << "   SEG   TAG    COORDINATES OF SEG CENTER     SEG          CHARGE DENSITY (COULOMBS/METER)"
				<< endl;
			os << "   NO:   NO:     X         Y         Z       LENGTH     REAL      IMAGINARY     MAGN        PHASE"
				<< endl;
			...
			for (int i = q_density_last_printed; i < q_density_nb_elements; i++)
			{
				oh.start_record();
				oh.padding(" ");
				oh.int_out(5, _q_density_segment_number[i]);
				oh.separator();
				oh.int_out(4, _q_density_segment_tag[i]);
				oh.separator();
				oh.real_out(9, 4, _q_density_segment_center_x[i], false);
				oh.separator();
				oh.real_out(9, 4, _q_density_segment_center_y[i], false);
				oh.separator();
				oh.real_out(9, 4, _q_density_segment_center_z[i], false);
				oh.separator();
				oh.real_out(9, 5, _q_density_segment_length[i], false);
				oh.separator();
				oh.real_out(11, 4, real(_q_density[i]), true);
				oh.separator();
				oh.real_out(11, 4, imag(_q_density[i]), true);
				oh.separator();
				oh.real_out(11, 4, abs(_q_density[i]), true);
				oh.separator();
				oh.real_out(9, 3, arg_degrees(_q_density[i]), false);
				oh.separator();
				oh.end_record();
			}
	...
	if (m_geometry->m != 0) {
		...
		os << "    REAL   IMAGINARY    REAL   IMAGINARY    REAL   IMAGINARY"
			<< endl;

		for (int i = 0; i < patch_nb_elements; i++) {
			oh.start_record();
			oh.padding(" ");
			oh.int_out(4, _patch_number[i]);
			oh.separator();
			oh.real_out(7, 3, _patch_center_x[i], false);
			oh.separator();
			oh.real_out(7, 3, _patch_center_y[i], false);
			oh.separator();
			oh.real_out(7, 3, _patch_center_z[i], false);
			oh.separator();
			oh.real_out(11, 4, abs(_patch_tangent_vector1[i]), true);
			oh.separator();
			oh.real_out(8, 2, arg_degrees(_patch_tangent_vector1[i]), false);
			oh.separator();
			oh.real_out(11, 4, abs(_patch_tangent_vector2[i]), true);
			oh.separator();
			oh.real_out(8, 2, arg_degrees(_patch_tangent_vector2[i]), false);
			oh.separator();
			oh.real_out(9, 2, real(_patch_e_x[i]), true);
			oh.separator();
			oh.real_out(9, 2, imag(_patch_e_x[i]), true);
			oh.separator();
			oh.real_out(9, 2, real(_patch_e_y[i]), true);
			oh.separator();
			oh.real_out(9, 2, imag(_patch_e_y[i]), true);
			oh.separator();
			oh.real_out(9, 2, real(_patch_e_z[i]), true);
			oh.separator();
			oh.real_out(9, 2, imag(_patch_e_z[i]), true);
			oh.separator();
			oh.end_record();
		}/*for(int i=0; i<patch_nb_elements; i++)*/
